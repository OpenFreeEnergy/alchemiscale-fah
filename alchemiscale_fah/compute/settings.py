...
class FahSynchronousComputeServiceSettings(BaseModel):
    ...
    name: str = Field(
        ..., description=("The name to give this compute service; used for Task provenance, so "
    ...
    keep_shared: bool = Field(
            False, description="If True, don't remove shared directories for `ProtocolDAG`s after completion."
    )
    keep_scratch: bool = Field(
            False, description="If True, don't remove scratch directories for `ProtocolUnit`s after completion."
    )
    ...
    scopes: Optional[List[Scope]] = Field(
            None, description="Scopes to limit Task claiming to; defaults to all Scopes accessible by compute identity."
    )
    claim_limit: int = Field(
            1, description="Maximum number of Tasks to claim at a time from a TaskHub."
    )
    loglevel: str = Field(
            "WARN", description="The loglevel at which to report; see the :mod:`logging` docs for available levels."
    )
    logfile: Optional[os.PathLike] = Field(
            None, description="Path to file for logging output; if not set, logging will only go to STDOUT."
    )
    client_max_retries: int = Field(
            5, description=("Maximum number of times to retry a request. "
    ...
    client_retry_base_seconds: float = Field(
            2.0, description="The base number of seconds to use for exponential backoff. Must be greater than 1.0."
    )
    client_retry_max_seconds: float = Field(
            60.0, description="Maximum number of seconds to sleep between retries; avoids runaway exponential backoff while allowing for many retries."
    )
    client_verify: bool = Field(
            True, description="Whether to verify SSL certificate presented by the API server."
    )
