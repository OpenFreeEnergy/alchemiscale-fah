...
from alchemiscale_fah.protocols.feflow.nonequilibrium_cycling import FahNonEqulibriumCyclingSimulationUnit, FahNonEqulibriumCyclingProtocol

from alchemiscale_fah.tests.integration.compute.conftest import generate_tyk2_solvent_network
...
class TestFahNonEqulibriumCyclingProtocol:
    ...
    async def test_dag_execute(self, fah_client_preloaded, transformation):
        ...
        t_sk = ScopedKey(gufe_key=transformation.key, org='test_org', campaign='test_campaign', project='test_project')
        task_sk = ScopedKey(gufe_key=GufeKey('Task-12345'), org='test_org', campaign='test_campaign', project='test_project')

        index = FahComputeServiceIndex(Path('./index/index_dir'), Path('./index/object_store'))

        # execute DAG; "work server" will "finish" a simulation unit after a preset amount of time
        pdr = await execute_DAG(protocoldag,
